...
const createTrack = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error.meta.cause === "No 'Vacancy' record(s) (needed to inline the relation on 'Track' record(s)) was found for a nested connect on one-to-many relation 'TrackToVacancy'.") {
      return res.status(400).json('Vacancy not found');
    }

    if (error.meta.cause === "No 'Recruiter' record(s)(needed to inline the relation on 'Track' record(s)) was found for a nested connect on one - to - many relation 'RecruiterToTrack'.") {
      return res.status(400).json('Recruitant not found');
    }
    console.log('unknown error in createTrack controller', error);
    return res.status(500).json('unknown error in createTrack controller: '+error);
  }
}
